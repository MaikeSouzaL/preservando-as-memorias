...
export async function GET() {
  ...
  const isOwner = Boolean(session?.isDevAdmin);

  // `config` é lido pelas telas públicas (preços, planos), mas carrega junto os
  // convites de funerária. Vazá-los permite se cadastrar com comissão zerada e
  // ainda queimar o convite legítimo. Só o dono vê a config completa.
  const { funeralHomeInvites, ...publicConfig } = data.config;

  return NextResponse.json({
    config: isOwner ? data.config : publicConfig,
    ...(isOwner ? { orders: data.orders } : {}),
  });
}
...
export async function PATCH(request: Request) {
  try {
    ...
    if (target === "moderation") {
      const autoApprove = Boolean(body.autoApproveTributes);
      const updatedModeration = await updatePlatformData((data) => {
        data.config.autoApproveTributes = autoApprove;
        return data.config;
      });
      return NextResponse.json({ config: updatedModeration });
    }
    ...
    const updated = await updatePlatformData((data) => {
      if (target === "prices") {
        ...
        if (body.candlePriceCents !== undefined) {
          const candleCents = Math.round(Number(body.candlePriceCents));
          if (!Number.isFinite(candleCents) || candleCents < 0)
            throw new Error("Informe um valor válido para o preço da vela.");
          data.config.candlePriceCents = candleCents;
        }
      }
