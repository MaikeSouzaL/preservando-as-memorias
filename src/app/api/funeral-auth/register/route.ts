...
export async function POST(request: Request) {
  const limited = await checkRateLimit(request as any, "rl:funeral-register", {
    limit: 5,
    windowSecs: 60 * 60,
  });
  ...
  try {
    ...
    const funeralHome = await updatePlatformData((data) => {
      ...
      const newFuneralHome = {
        // funeral_homes.id é uuid no banco. O formato antigo `fh_<base36>` era
        // rejeitado pelo Postgres, e como o erro era engolido o cadastro
        // respondia 201 sem nunca ter salvado nada.
        id: randomUUID(),
        name,
        slug,
        contactName,
