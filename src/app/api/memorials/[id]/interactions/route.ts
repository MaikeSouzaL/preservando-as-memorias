  ...
  try {
    ...
    const created = await updatePlatformData((data) => {
      ...
      if (type === "tribute") {
        ...
        // Configurável pelo dono em Painel → Moderação (platform_config.auto_approve_tributes).
        const autoApprove = data.config.autoApproveTributes ?? true;
        const tribute = {
          ...
          status: (autoApprove ? "aprovada" : "pendente") as "aprovada" | "pendente",
          isPinned,
