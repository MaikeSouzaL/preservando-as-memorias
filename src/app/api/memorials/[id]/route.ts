...
export async function GET(_request: Request, context: MemorialRouteContext) {
  ...
  // O ID do memorial está no QR Code afixado no túmulo, ou seja, é público por
  // natureza. O endereço residencial de entrega da placa não pode sair junto.
  const canSeePrivateFields = canManageMemorial(session, memorial.ownerId);
  const { deliveryAddress, ...publicMemorial } = memorial;

  return NextResponse.json({
    memorial: canSeePrivateFields ? memorial : publicMemorial,
