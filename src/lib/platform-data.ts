...
export type FuneralHome = {
  ...
  /** FK para funeral_billing_plans.id — override do plano de cobrança desta funerária. null = herda o plano padrão. */
...
function mapFuneralHome(r: any): FuneralHome {
  return {
    ...
    billingPlanId: r.billing_plan_id ?? null,
    createdAt: r.created_at,
    updatedAt: r.updated_at,
  };
}
...
export async function readPlatformData(): Promise<PlatformData> {
  ...
  const config: PlatformConfig = {
    ...
    autoApproveTributes: cfg.auto_approve_tributes ?? true,
  };
...
async function persistChanges(original: PlatformData, updated: PlatformData): Promise<void> {
  ...
  // As escritas são agrupadas em ondas por dependência de chave estrangeira.
  // Rodar tudo num único Promise.all cria corrida: o qr_code chega antes do
  // memorial existir e o insert é rejeitado pela FK.
  const ops: PromiseLike<any>[] = []; // onda 1 — sem dependências
  const opsL2: PromiseLike<any>[] = []; // onda 2 — depende da onda 1
  const opsL3: PromiseLike<any>[] = []; // onda 3 — depende da onda 2

  // ── Memorials ──
  const mDiff = diffItems(original.memorials, updated.memorials);
  if (mDiff.added.length > 0) opsL2.push(supabase.from("memorials").insert(mDiff.added.map(toDbMemorial)));
  for (const m of mDiff.modified) opsL2.push(supabase.from("memorials").update(toDbMemorial(m)).eq("id", m.id));
  if (mDiff.deleted.length > 0) ops.push(supabase.from("memorials").delete().in("id", mDiff.deleted));

  // ── QR Codes ── (FK → memorials)
  const qDiff = diffItems(original.qrCodes, updated.qrCodes);
  if (qDiff.added.length > 0) opsL3.push(supabase.from("qr_codes").insert(qDiff.added.map(toDbQrCode)));
  for (const q of qDiff.modified) opsL3.push(supabase.from("qr_codes").update(toDbQrCode(q)).eq("id", q.id));
  if (qDiff.deleted.length > 0) ops.push(supabase.from("qr_codes").delete().in("id", qDiff.deleted));

  // ── Tributes ── (FK → memorials)
  const tDiff = diffItems(original.tributes, updated.tributes);
  if (tDiff.added.length > 0) opsL3.push(supabase.from("tributes").insert(tDiff.added.map(toDbTribute)));
  for (const t of tDiff.modified) opsL3.push(supabase.from("tributes").update(toDbTribute(t)).eq("id", t.id));
  if (tDiff.deleted.length > 0) ops.push(supabase.from("tributes").delete().in("id", tDiff.deleted));

  // ── Candles ── (FK → memorials)
  const cDiff = diffItems(original.candles, updated.candles);
  if (cDiff.added.length > 0) opsL3.push(supabase.from("candles").insert(cDiff.added.map(toDbCandle)));
  if (cDiff.deleted.length > 0) ops.push(supabase.from("candles").delete().in("id", cDiff.deleted));

  // ── Orders ── (FK → memorials, funeral_homes, profiles)
  const oDiff = diffItems(original.orders, updated.orders);
  if (oDiff.added.length > 0) opsL3.push(supabase.from("orders").insert(oDiff.added.map(toDbOrder)));
  for (const o of oDiff.modified) opsL3.push(supabase.from("orders").update(toDbOrder(o)).eq("id", o.id));
    ...
    ops.push(
      supabase
        ...
        .update({
          ...
          // NOTA: `qr_delivery_mode` foi removido deste payload — a coluna nunca existiu
          // na tabela `funeral_homes` e sua presença aqui fazia QUALQUER atualização
          // (aprovar, rejeitar, comissão, plano) falhar silenciosamente com erro do
          // Postgres ("column does not exist"), capturado só pelo runWave() acima.
          ...
          billing_plan_id: fh.billingPlanId ?? null,
        })
        .eq("id", fh.id)
    );
  }

  // ── Offer Links ── (FK → funeral_homes)
  ...
  if (JSON.stringify(original.config) !== JSON.stringify(updated.config)) {
    ops.push(
      supabase.from("platform_config").update({
        ...
        auto_approve_tributes: updated.config.autoApproveTributes ?? true,
      }).eq("id", 1)
    );
  }
  ...
  // supabase-js resolve com { error } em vez de rejeitar. Sem esta checagem
  // toda escrita que falha some em silêncio e a API ainda responde 201.
  async function runWave(wave: PromiseLike<any>[], label: string) {
    if (wave.length === 0) return;
    const results = await Promise.all(wave);
    const failures = results.map((r: any) => r?.error).filter(Boolean);

    if (failures.length > 0) {
      console.error(`[platform-data] falha ao persistir (${label}):`, failures);
      throw new Error(
        `Não foi possível salvar as alterações: ${failures.map((e: any) => e.message).join(" | ")}`
      );
    }
  }

  await runWave(ops, "base");
  await runWave(opsL2, "memoriais");
  await runWave(opsL3, "dependentes");
}
